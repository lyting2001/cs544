...
if __name__=='__main__':
  ...
  texts = [clean_text(s['x']) for s in sons]
  ## straight up
  # cfs = np.array([[t for i,t in enumerate(s['cf'] )
  #           if (t!='<padding>' and t!='<start>' and t!='<end>')
  #                 ] for s in sons])
  cfs = [clean_text(s['cf']) for s in sons]
  ...
  for i in range(len(sons)):
      if targs.aspect=='0':
        ## aspect 0
        if flipit:
          istr.append(str(1-ys[i])+'\t'+texts[i])### FLIP?!?! ## you flip y cuz the labels are maybe flipped, the cf label is always true
        else:
          istr.append(str(ys[i])+'\t'+texts[i])### FLIP?!?!
        istr.append(str(ys[i])+'\t'+cfs[i])
  ...
  with open('./cf_flipdump_results.txt','a') as f:
